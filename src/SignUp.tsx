...
export const SignUp: React.FC = () => {
  ...
  const handleSocialSignUp = (provider: string) => {
    triggerToast(`Establishing secure Oauth connection for registration with ${provider}...`, 'info');
    setTimeout(() => {
      setFullName(`${provider} User`);
      setEmail(`${provider.toLowerCase()}_user@nestlist.ke`);
      setPhone('254712345678');
      setPassword('Premium@123');
      setConfirmPassword('Premium@123');
      setAcceptedTerms(true);
      setAcceptedPrivacy(true);
      triggerToast(`${provider} details pre-filled. Complete registration setup!`, 'success');
    }, 1200);
  };
